...
- (void)copyDocumentsFromAssets :(NSString *)dpath
{
    ...
    for (NSString *fpath in [[NSFileManager defaultManager] contentsOfDirectoryAtPath:hf error:nil]) {
        if([fpath.pathExtension isEqualToString:@"pdf"] || [fpath.pathExtension isEqualToString:@"PDF"] ||
           [fpath.pathExtension isEqualToString:@"docx"] || [fpath.pathExtension isEqualToString:@"DOCX"])
        {
...
- (UIImage *)setImageForCellAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    PDF_DOC m_docThumb = PDF_Document_open(cpath,nil, &err);
    [RDPDFDoc setOpenFlag:1];
    int result = 1;
    if( m_docThumb == NULL )
    {
        switch( err )
    ...
    {
        PDF_PAGE page = PDF_Document_getPage(m_docThumb, 0);
        float w = PDF_Document_getPageWidth(m_docThumb,0);
        float h = PDF_Document_getPageHeight(m_docThumb,0);
        PDF_DIB m_dib = NULL;
        int iw = 200;
        int ih = 200;
        ...
        PDF_Page_renderPrepare( page, bmp );
        PDF_Page_render(page, bmp, mat,false,1);
        Matrix_destroy(mat);
        PDF_Page_close(page);
        PDF_Document_close(m_docThumb);
...
- (void)pdf_open_path:(RDFileItem *)item :(NSString *)pswd
{
    ...
    {
        if(m_pdf == nil) m_pdf = [[UIStoryboard storyboardWithName:@"PDFReaderCtrl" bundle:nil] instantiateViewControllerWithIdentifier:@"rdpdfreader"];
        [m_pdf setDoc:doc];
    ...
    {
        DOCXDoc *docx = [[DOCXDoc alloc] init];
        [item.locker lock];
        result = [docx open:item.path :pswd];
        [item.locker unlock];
        if(result == 0)//succeeded, but if result != 0 or 2?
        {
            DOCXReaderCtrl *docxCtrl = [[UIStoryboard storyboardWithName:@"DOCXReaderCtrl" bundle:nil] instantiateViewControllerWithIdentifier:@"rddocxreader"];
            [docxCtrl setDoc:docx];
            docxCtrl.hidesBottomBarWhenPushed = YES;
            GLOBAL.g_pdf_name = [NSMutableString stringWithString:[item.path lastPathComponent]];
            [self.navigationController pushViewController:docxCtrl animated:YES];
        }
        else if(result == 2)//require password
        {
            [self pdf_show_pwd_error:^(NSString *pwd) {
