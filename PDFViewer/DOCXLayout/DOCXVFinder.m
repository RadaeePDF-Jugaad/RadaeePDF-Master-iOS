...
-(void)drawSingleOffScreen :(RDVCanvas *)canvas :(DOCXVPage *)page :(int)docx :(int)docy :(int)index :(unsigned)color
{
    if( !is_cancel )
    {
        [m_eve wait];
    ...
    if( m_finder != NULL && index >= 0 && index < m_page_find_cnt )
    {
...
-(void)find_draw:(RDVCanvas *)canvas :(DOCXVPage *)page :(int)index :(unsigned)color//draw current found
{
    if( !is_cancel )
    {
        [m_eve wait];
...
- (void)find_draw_all:(RDVCanvas *)canvas :(DOCXVPage *)page
{
    if (DRAW_ALL == 1) {
        for(int index = 0; index < m_page_find_cnt; index++) {
            if (index == m_page_find_index)
