...
-(float)ToDOCXX :(int) vx :(int) scrollx
{
    float dibx = scrollx + vx - m_x;
    return dibx / m_scale;
}
-(float)ToDOCXY :(int) vy :(int) scrolly
{
...
-(void)blocks_destroy :(DOCXThread *) thread
{
    if (!m_caches) return;
    int xcur = 0;
    int ycur = 0;
...
-(void)blocks_create;
{
    ...
    if(xcnt <= 0) xcnt = 1;
    if(ycnt <= 0) ycnt = 1;
    m_caches = [[DOCXCacheSet alloc] init :xcnt :ycnt];
    DOCXCache *cache;
    int xcur;
    int ycur;
    int yval = 0;
    ...
    for( ycur = 0; ycur < ycnt - 1; ycur++ )
    {
        ...
        {
            cache = [[DOCXCache alloc] init :m_doc :m_pageno :m_scale :xval :yval :m_cw :m_ch];
            cache.thumbMode = m_thumb;
            [m_caches set :xcur :ycur :cache];
            xval += m_cw;
        }
        cache = [[DOCXCache alloc] init :m_doc :m_pageno :m_scale :xval :yval :m_w - xval :m_ch];
        cache.thumbMode = m_thumb;
        [m_caches set :xcur :ycur :cache];
        yval += m_ch;
    }
    ...
    for(xcur = 0; xcur < xcnt - 1; xcur++)
    {
        cache = [[DOCXCache alloc] init :m_doc :m_pageno :m_scale :xval :yval :m_cw :m_h - yval];
        cache.thumbMode = m_thumb;
        [m_caches set :xcur :ycur :cache];
        xval += m_cw;
    }
    cache = [[DOCXCache alloc] init :m_doc :m_pageno :m_scale :xval :yval :m_w - xval :m_h - yval];
    cache.thumbMode = m_thumb;
    [m_caches set :xcur :ycur :cache];
}
...
-(void)vClips :(DOCXThread *) thread :(bool) clip;
{
    if(m_need_clip)
    {
        ...
        {
            m_caches = [[DOCXCacheSet alloc] init :1 :1];
            DOCXCache *cache = [[DOCXCache alloc] init :m_doc :m_pageno :m_scale :0 :0 :m_w :m_h];
            cache.thumbMode = m_thumb;
...
-(void)vEndPage :(DOCXThread *) thread
{
    ...
    for(ycur = 0; ycur < ycnt; ycur++)
    {
        for(xcur = 0; xcur < xcnt; xcur++)
        {
            DOCXCache *cache = [m_caches get :xcur :ycur];
...
-(NSMutableArray *)vBackCache
{
    ...
    if (m_caches_zoom)
    {
        DOCXCacheSet *zcaches = m_caches_zoom;
        m_caches_zoom = nil;
        int xcur = 0;
        int ycur = 0;
        ...
        for(ycur = 0; ycur < ycnt; ycur++)
        {
            for(xcur = 0; xcur < xcnt; xcur++)
            {
                DOCXCache *cache = [zcaches get :xcur :ycur];
                [ret addObject:cache];
            }
        }
    }
...
-(void)end_render_xb :(DOCXThread *) thread :(int) xb0 :(int) xb1 :(int) yb
{
    while(xb0 < xb1)
    {
        DOCXCache *cache = [m_caches get :xb0 :yb];
...
-(void)end_render_yb :(DOCXThread *) thread :(int) yb0 :(int) yb1 :(int) xcnt
{
    while(yb0 < yb1)
    {
...
-(void)vRenderAsync :(DOCXThread *) thread :(int) vx :(int) vy :(int) vw :(int) vh;
{
    if(!m_caches) return;
    int xcnt = [m_caches cols];
    int ycnt = [m_caches rows];
    ...
    for(int yval = y0; yval < m_y1 && ycur < ycnt; ycur++)
    {
        ...
        {
            DOCXCache *vc = [m_caches get :xcur :ycur];
...
-(void)vRenderSync :(DOCXThread *) thread :(int) vx :(int) vy :(int) vw :(int) vh;
{
    if (!m_caches) return;
    int xcnt = [m_caches cols];
    int ycnt = [m_caches rows];
...
-(void)vDraw :(DOCXThread *) thread :(int) vx :(int) vy :(int) vw :(int) vh
{
    if (!m_caches) return;
...
-(bool)vDrawZoom :(float)scale
{
    ...
    for(ycur = 0; ycur < ycnt; ycur++)
    {
        for(xcur = 0; xcur < xcnt; xcur++)
        {
            DOCXCache *vc = [m_caches_zoom get :xcur :ycur];
