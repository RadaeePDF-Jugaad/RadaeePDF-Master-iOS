...
-(DOCXCache *)vClone
{
    DOCXCache *vc = [[DOCXCache alloc] init :m_doc :m_pageno :m_scale :m_dibx :m_diby :m_dibw :m_dibh];
...
-(void)vRender
{
    ...
    //NSLog(smsg);
    DOCXPage *page = [m_doc page :m_pageno];
    DOCX_Page_renderPrepare([page handle], dib);
    if(m_status < 0)
    {
        Global_dibFree(dib);
        return;
    ...
    DOCX_Page_render([page handle],  dib, m_scale, m_dibx, m_diby, 2);
...
void DOCXDataProviderReleaseDataCallback(void *info, const void *data, size_t size)
{
...
-(CALayer *)ProLayer
{
    ...
    m_dib = NULL;
    CGDataProviderRef provider = CGDataProviderCreateWithData(dib, Global_dibGetData(dib), m_dibw * m_dibh * 4, DOCXDataProviderReleaseDataCallback);
...
-(DOCXCache *)get :(int)col :(int) row
{
    ...
    return (DOCXCache *)[colo objectAtIndex :row];
}
