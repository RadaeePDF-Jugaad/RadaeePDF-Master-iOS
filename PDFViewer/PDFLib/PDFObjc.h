...
@interface RDPDFEditNode : NSObject
{
    PDF_EDITNODE m_node;
}
@property (readonly)PDF_EDITNODE handle;
...
/**
 * @brief get text char index nearest to point.
 * @param x point as [x,y] in PDF coordinate.
 * @param y point as [x,y] in PDF coordinate.
 * @return char index or -1 failed.
 */
 ...
 * @brief get image information
 * @param index range:[0, ObjsGetCharCount()-1]
 * @param info 10 elements int array, return informatiion of image.
** @return true or false
 */
-(bool)objsGetImageInfo:(int)index : (int*)info;
/**
 ...
 * @param index range:[0, ObjsGetCharCount()-1]
 * @param data null if char is not image, others: pixels data and information, array like [(bits<<16)|number_pals, stride, width, height, palette, pixels...]<br/>
 * bits is bits number, must be 8 or 32.<br/>
 * number_pals is number of palette entries.<br/>
 * stride is number of ints per pixel line.<br/>
 * palette is palette data. if number_pals is 0, this item is ignored.<br/>
 ...
 */
-(bool)objsSetImageJPEGByMem:(int)index :(const unsigned char*)data :(int)len :(bool)interpolate;
/**
...
/**
 * @brief import a page to dest page.
 * a premium license is required for this method.
 * @param src_no 0 based page NO. from source Document that passed to ImportStart.
 * @param dst_no 0 based page NO. to insert in dest document object.
 * @param rect. src_no Page object will draw on dst_no Page, in rect in dst_no Page coordinate.
 * @return true or false.
*/
