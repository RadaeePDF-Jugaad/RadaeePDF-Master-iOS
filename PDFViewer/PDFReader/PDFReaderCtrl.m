...
- (void)loadPDF
{
    ...
    m_view = [mView view];
    [self thumbInit];
    [m_view PDFOpen:m_doc :4 :[mView canvas] :self];
    [m_thumb PDFOpen:m_doc :2 :[mThumb canvas] :self];
...
- (void)setDoc:(RDPDFDoc *)doc :(int)pageno :(BOOL)readonly
{
    ...
    m_view = [mView view];
    m_thumb = (PDFThumbView *)[mThumb view];
    [m_view PDFOpen:m_doc :4 :[mView canvas] :self];
    [m_thumb PDFOpen:m_doc :2 :[mThumb canvas] :self];
...
- (void)thumbInit
{
    if (!GLOBAL.g_navigation_mode) {
        mSlider.minimumValue = 1;
        mSlider.maximumValue = m_doc.pageCount;
        mSlider.value = self.PDFCurPage + 1;
        mSliderLabel.text = [NSString stringWithFormat:@"%i/%i", self.PDFCurPage, m_doc.pageCount];
        [mSlider addTarget:self action:@selector(OnSliderValueChange:forEvent:) forControlEvents:UIControlEventValueChanged];
    } else {
        m_thumb = (PDFThumbView *)[mThumb view];
    }
    
    if (GLOBAL.g_navigation_mode) {
         [self->mBarThumbButton setImage:[UIImage imageNamed:@"btn_thumb"]];
    } else {
        [self->mBarThumbButton setImage:[UIImage imageNamed:@"btn_slider"]];
    }
...
{
    if (!GLOBAL.g_navigation_mode) {
           mSlider.value = pageno +1;
           [self setSliderText:(int)mSlider.value];
       } else {
...
- (void)enter_none
{
    [mBarNoneTop setHidden:NO];
    [mBarNoneBottom setHidden:NO];
    [mBarAnnot setHidden:YES];
    [mBarSearchTop setHidden:YES];
    [mBarSearchBottom setHidden:YES];
    if (!GLOBAL.g_navigation_mode) {
        [mThumb setHidden:YES];
        [mSliderView setHidden:NO];
    } else {
        [mThumb setHidden:NO];
        [mSliderView setHidden:YES];
    }
}

- (void)enter_annot
{
    [mBarNoneTop setHidden:YES];
    [mBarNoneBottom setHidden:YES];
    [mBarAnnot setHidden:NO];
    [mBarSearchTop setHidden:YES];
    [mBarSearchBottom setHidden:YES];
    [mThumb setHidden:YES];
    mBarAnnotDoneButton.enabled = YES;
    if (m_annot_type == 2 || m_annot_type == 5) {
        mBarAnnotColorButton.enabled = NO;
    } else {
        mBarAnnotColorButton.enabled = YES;
    }
}
...
- (void)enter_search
{
    CGRect rect = mBarNoneTop.frame;
    mBarSearchTop.frame = rect;
    [mBarNoneTop setHidden:YES];
    [mBarNoneBottom setHidden:YES];
    [mBarAnnot setHidden:YES];
    [mBarSearchTop setHidden:NO];
    [mBarSearchBottom setHidden:NO];
    [mThumb setHidden:YES];
    
    [mSearchText becomeFirstResponder];
    [mSearchText addTarget:self action:@selector(search_forward:) forControlEvents:UIControlEventEditingDidEndOnExit];
    ...
    if ([mSearchText isFirstResponder]) {
        [UIView animateWithDuration:0.33 delay:0 options:UIViewAnimationOptionTransitionCurlUp animations:^{
           if (keyboardFrameBeginRect.origin.y > keyboardFrameEndRect.origin.y) {
               self->mBarSearchBottom.transform = CGAffineTransformMakeTranslation(0, - keyboardFrameEndRect.size.height);
           } else {
               self->mBarSearchBottom.transform = CGAffineTransformIdentity;
           }
        } completion:nil];
    }
}
...
- (void)enter_select
{
    ...
    [mBarSearchBottom setHidden:YES];
    [mThumb setHidden:YES];
    mBarAnnotDoneButton.enabled = NO;
    mBarAnnotColorButton.enabled = NO;
    m_annot_type = 100;
}
...
-(void)viewWillAppear:(BOOL)animated
{
    ...
    // Set images
    if(_closeImage)
        mBtnBack.image = _closeImage;
    if(_doneImage)
        mBtnDone.image = _doneImage;
    if(_removeImage)
        mBtnCancel.image = _removeImage;
    if(_prevImage)
        mBtnPrev.image = _prevImage;
    if(_nextImage)
        mBtnNext.image = _nextImage;
    
    fileName.text = [GLOBAL.g_pdf_name stringByDeletingPathExtension];
    fileName.textColor = mBarNoneTop.tintColor;
...
- (void)viewWillTransitionToSize:(CGSize)size withTransitionCoordinator:(id<UIViewControllerTransitionCoordinator>)coordinator {
    [m_view pageRotationRefreshContentSize];
...
- (IBAction)mode_pressed:(id)sender
{
    ...
    MenuVMode *view = [[MenuVMode alloc] init:CGPointMake(mBarNoneBottom.frame.origin.x, mBarNoneBottom.frame.origin.y) :^(int vmode){
        switch(vmode)
        {
            case 1:
                [vw PDFSetVMode:1];
...
-(void)OnOutline
{
    ...
    {
        [self show_error:NSLocalizedString(@"This PDF file has no outlines!", nil)];
        return;
    }
...
- (void)thumb_pressed:(id)sender
{
    ...
    [UIView animateWithDuration:0.33 delay:0 options:UIViewAnimationOptionTransitionCurlUp animations:^{
        if (self->showingThumb) {
            self->mSliderView.transform = CGAffineTransformIdentity;
            self->mThumb.transform = CGAffineTransformIdentity;
            self->mBarNoneBottom.transform = CGAffineTransformIdentity;
            self->showingThumb = NO;
        } else {
            self->mSliderView.transform = CGAffineTransformMakeTranslation(0, -self->mSliderView.frame.size.height - safeAreaBottom);
            self->mBarNoneBottom.transform = CGAffineTransformMakeTranslation(0,-self->mSliderView.frame.size.height - safeAreaBottom);
            self->mThumb.transform = CGAffineTransformMakeTranslation(0, -self->mThumb.frame.size.height - safeAreaBottom);
            self->mBarNoneBottom.transform = CGAffineTransformMakeTranslation(0,-self->mThumb.frame.size.height - safeAreaBottom);
...
- (IBAction)tool_pressed:(id)sender
{
    ...
    MenuTool *view = [[MenuTool alloc] init:CGPointMake(mBarNoneBottom.frame.origin.x, mBarNoneBottom.frame.origin.y) :^(int tool){
        [thiz->m_popup dismiss];
        switch(tool)
        {
            case 0:
                    ...
                    [pages setCallback:self->m_doc :^(const bool *pages_del, const int *pages_rot)
                    {
                        ...
                        while(pcur > 0)
                        {
                            ...
                            else if((pages_rot[pcur] >> 16) != (pages_rot[pcur] & 0xffff))
                            {
                                ...
                                [page updateWithPGEditor];//required for page editing mode.
                                page = nil;
            ...
            case 8://eding mode
                ...
                break;
            case 9://show attachments
                if ([self->m_doc getEmbedFileCount] >= 0) {
                    RDEmbeddedFilesViewController *embeddedFilesViewController = [[RDEmbeddedFilesViewController alloc] initWithNibName:@"RDEmbeddedFilesViewController" bundle:nil];
                    embeddedFilesViewController.doc = self->m_doc;
                    embeddedFilesViewController.readonly = self->m_readonly;
                    embeddedFilesViewController.modalPresentationStyle = UIModalPresentationOverCurrentContext;
                    embeddedFilesViewController.modalTransitionStyle = UIModalTransitionStyleCrossDissolve;
                    [self presentViewController:embeddedFilesViewController animated:YES completion:nil];
                }
                break;
            default:
                break;
...
- (IBAction)annot_pressed:(id)sender
{
    if ([m_view canSaveDocument]) {
        ...
        MenuAnnot *view = [[MenuAnnot alloc] init:CGPointMake(mBarNoneBottom.frame.origin.x, mBarNoneBottom.frame.origin.y) :^(int type){
        ...
        [view updateVisible:_hideDrawImage];
...
- (IBAction)search_result_pressed:(id)sender
{
...
#ifdef FTS_ENABLED
    ftsTableViewController = [[FTSSearchTableViewController alloc] initWithNibName:@"FTSSearchTableViewController" bundle:nil];
    ftsTableViewController.docId = [RDUtils getPDFIDForDoc:m_doc];
    ftsTableViewController.searchedText = mSearchText.text;
    ftsTableViewController.delegate = self;
    ftsTableViewController.modalPresentationStyle = UIModalPresentationOverCurrentContext;
    ftsTableViewController.modalTransitionStyle = UIModalTransitionStyleCoverVertical;
    [self presentViewController:ftsTableViewController animated:YES completion:nil];
#else
    ...
    viewController.searchedString = mSearchText.text;
...
{
    MenuSearch *view = [[MenuSearch alloc] init:CGPointMake(mBarNoneBottom.frame.origin.x, mBarNoneBottom.frame.origin.y) :nil];
    m_popup = [[PDFPopupCtrl alloc] init:view];
    [self presentViewController:m_popup animated:YES completion:nil];
}
...
{
    [ftsTableViewController dismissViewControllerAnimated:YES completion:nil];
    [self PDFGoto:occurrence.page];
    [m_view applyFTSOccurrence:occurrence];
}
#endif
...
- (RDMenu *)getScrollViewWithMenu:(RDMenu *)menu
{
    ...
    CGFloat height = self.view.frame.size.height - y - (self.view.frame.size.height - mBarNoneBottom.frame.origin.y) - 10;
...
- (void)setSliderText:(int)value {
    mSliderLabel.text = [NSString stringWithFormat:@"%i/%i", value, m_doc.pageCount];
    mSliderLabel.textColor = [UIColor blackColor];
}
...
-(void)initialPopupView
{
    if(!m_readonly)
    {
        ...
        UIMenuItem *removeObjs = [[UIMenuItem alloc] initWithTitle:@"Erase" action:@selector(objsRemove:)];
        NSArray *itemsMC = [[NSArray alloc] initWithObjects:underline, highline, strike, squiggly, copyText, removeObjs, nil];
...
-(void)objsRemove:(id)sender
{
    [self->m_view vSelRemove];
    [self endSelect];
}
...
- (void)OnFound:(bool)found
{
    if (_delegate && [_delegate respondsToSelector:@selector(didSearchTerm:found:)]) {
        [_delegate didSearchTerm:mSearchText.text found:found];
    }
...
- (void)OnAnnotEditBox:(RDPDFAnnot *)annot :(CGRect)annotRect :(NSString *)editText :(float)textSize
{
    ...
    if([annot getEditType] == 3)//multi-line
    {
        ...
        CGRect rect = [mView convertRect:annotRect toView: pop.view];
        text.frame = rect;
        text.text = editText;
...
- (void)searchProgress{
    ...
    {
        NSLog(@"index: %i pcnt:%i",index,pcnt);
    ...
    m_rdvfinder.cancelBlock = ^(bool result){
        [self dismissViewControllerAnimated:NO completion:^{
            if(!result)
            {
                [self show_error:NSLocalizedString(@"No more found!", nil)];
            }
...
- (CGRect)buttonRect:(UIBarButtonItem *)item {
    ...
    CGRect fromRect = [mBarNoneTop convertRect:v.frame toView:self.view];
