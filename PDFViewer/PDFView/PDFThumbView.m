...
-(BOOL)PDFOpen :(RDPDFDoc *)doc :(int)page_gap :(RDPDFCanvas *)canvas :(id<PDFThumbViewDelegate>)del
{
    ...
    self.backgroundColor = [UIColor colorWithRed:0.8f green:0.8f blue:0.8f alpha:0.5f];
    [self setNeedsDisplay];
...
-(void)onDrawOffScreen:(CGContextRef)ctx
{
    ...
    while (start <= end)
    {
        ...
        if (!m_color) m_color = [UIColor colorWithRed:1.0f green:0.0f blue:0.0f alpha:1.0f];
        if (!m_pstyle)
        {
