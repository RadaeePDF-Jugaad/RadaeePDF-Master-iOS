...
//  Copyright © 2016 radaee. All rights reserved.
//
...
@implementation PDFLayoutView
{
    ...
    int m_aop_node;
    bool m_aop_attached;
    PDF_RECT m_aop_rect;//adjust size rect
    PDF_POINT m_aop_pt0[2];
    PDF_POINT m_aop_pts[2];
    int m_annot_type;
    PDF_POINT *m_aop_ppts;
    PDF_POINT *m_aop_ppt0;
    int m_aop_ppts_cnt;
}
...
-(BOOL)PDFOpen:(RDPDFDoc *)doc :(int)page_gap :(RDPDFCanvas *)canvas :(id<PDFLayoutDelegate>) del
{
    ...
    m_canvas = canvas;
    m_del = del;
    if (!m_aimg) m_aimg = [UIImage imageNamed:@"btn_annot_stamp"];
    m_dimg = nil;
...
-(void)PDFSetVMode:(int)vmode
{
    ...
    switch (m_save_vmode) {
        ...
        case 3:// Single Page (LTOR, paging enabled)
            ...
            [self pageRotationRefreshContentSize];
            break;
...
- (void)imageFromPage:(int)index inPDFRect:(CGRect )rect withScale:(float)scale {
    ...
    PDF_Page_renderPrepare(page.handle, bmp);
    PDF_Page_render(page.handle, bmp, mat, false, 2);
    Matrix_destroy(mat);
    page = nil;
...
+(void)osDrawBlk:(CGContextRef)context :(float)x :(float)y
{
    CGRect rect = CGRectMake(x - BLK_GAP, y - BLK_GAP, BLK_GAP * 2, BLK_GAP * 2);
    CGContextFillRect(context, rect);
    CGContextStrokeRect(context, rect);
}

+(void)osDrawLine:(CGContextRef)context :(float)x1 :(float)y1 :(float)x2 :(float)y2
{
...
+(bool)vOnBlk:(float)x :(float)y :(float)xb :(float)yb :(float)scale
{
    float gap = scale * BLK_GAP;
    return (x >= xb - gap && x <= xb + gap &&
            y >= yb - gap && y <= yb + gap);
}
+(bool)vInRect:(float)x :(float)y :(PDF_RECT)rect
{
    return (x >= rect.left && x <= rect.right &&
            y >= rect.top && y <= rect.bottom);
}

-(void)osDrawAnnotNormal:(CGContextRef)context
{
    CGContextSetLineWidth(context, 1);
    CGContextSetRGBStrokeColor(context, 0, 0, 0, 1);
    float scale = 1.0f / m_scale_pix;
    CGRect rect1 = CGRectMake(m_aop_rect.left * scale,
                              m_aop_rect.top * scale,
                              (m_aop_rect.right - m_aop_rect.left) * scale,
                              (m_aop_rect.bottom - m_aop_rect.top) * scale);
    CGContextStrokeRect(context, rect1);
}

-(void)osDrawAnnotResize:(CGContextRef)context
{
    CGContextSetLineWidth(context, 1);
    CGContextSetRGBStrokeColor(context, 0, 0, 0, 1);
    float scale = 1.0f / m_scale_pix;
    ...
    float l = rect1.origin.x;
    float t = rect1.origin.y;
    float r = l + rect1.size.width;
    float b = t + rect1.size.height;
    float mx = (l + r) * 0.5f;
    float my = (t + b) * 0.5f;
    [PDFLayoutView osDrawBlk:context :l :t];
    [PDFLayoutView osDrawBlk:context :mx :t];
    [PDFLayoutView osDrawBlk:context :r :t];
    [PDFLayoutView osDrawBlk:context :r :my];
    [PDFLayoutView osDrawBlk:context :r :b];
    [PDFLayoutView osDrawBlk:context :mx :b];
    [PDFLayoutView osDrawBlk:context :l :b];
    [PDFLayoutView osDrawBlk:context :l :my];
}

-(void)osDrawAnnotLine:(CGContextRef)context
{
    [self OnAnnotLoadLine];
    CGContextSetLineWidth(context, 1);
    CGContextSetRGBStrokeColor(context, 0, 0, 0, 1);
    float scale = 1.0f / m_scale_pix;
    CGRect rect1 = CGRectMake(m_aop_rect.left * scale,
                              ...
                              (m_aop_rect.bottom - m_aop_rect.top) * scale);
    CGContextStrokeRect(context, rect1);
    CGContextSetRGBStrokeColor(context, 0, 0, 1, 0.25f);
    [PDFLayoutView osDrawLine :context
                              :m_aop_pts[0].x * scale
                              :m_aop_pts[0].y * scale
                              :m_aop_pts[1].x * scale
                              :m_aop_pts[1].y * scale];
    ...
    CGContextSetRGBStrokeColor(context, 0, 0, 0, 1);
    CGContextSetRGBFillColor(context, 1, 1, 1, 1);
    [PDFLayoutView osDrawBlk:context :m_aop_pts[0].x * scale :m_aop_pts[0].y * scale];
    [PDFLayoutView osDrawBlk:context :m_aop_pts[1].x * scale :m_aop_pts[1].y * scale];
}

-(void)osDrawAnnotPoly:(CGContextRef)context
{
    [self OnAnnotLoadPath];
    CGContextSetLineWidth(context, 1);
    CGContextSetRGBStrokeColor(context, 0, 0, 0, 1);
    float scale = 1.0f / m_scale_pix;
    CGRect rect1 = CGRectMake(m_aop_rect.left * scale,
                              ...
                              (m_aop_rect.bottom - m_aop_rect.top) * scale);
    CGContextStrokeRect(context, rect1);
    if (!m_aop_ppts) return;
    CGContextSetRGBStrokeColor(context, 0, 0, 1, 0.25f);
    PDF_POINT *pend = m_aop_ppts + m_aop_ppts_cnt;
    CGContextBeginPath( context );
    PDF_POINT *pcur = m_aop_ppts;
    CGContextMoveToPoint(context, pcur->x * scale, pcur->y * scale);
    for (pcur++; pcur < pend; pcur++)
    {
        CGContextAddLineToPoint(context, pcur->x * scale, pcur->y * scale);
    }
    if (m_annot_type == 7) CGContextClosePath(context);
    CGContextStrokePath(context);

    CGContextSetRGBStrokeColor(context, 0, 0, 0, 1);
    CGContextSetRGBFillColor(context, 1, 1, 1, 1);
    for (pcur = m_aop_ppts; pcur < pend; pcur++)
    {
        [PDFLayoutView osDrawBlk:context :pcur->x * scale :pcur->y * scale];
    }
}
...
-(void)osDrawAnnot:(CGContextRef)context
{
    ...
    {
        if (m_aop_type == 1)
            [self osDrawAnnotResize:context];
        else if (m_aop_type == 2)
            [self osDrawAnnotLine:context];
        else if (m_aop_type == 3)
            [self osDrawAnnotPoly:context];
        else
            [self osDrawAnnotNormal:context];
    }
}

-(void)osDrawEdit:(CGContextRef)context
{
    if( m_status != sta_edit) return;
    CGContextSetLineWidth(context, 1);
    CGContextSetRGBStrokeColor(context, 0, 0, 0, 1);
    float scale = 1.0f / m_scale_pix;
    CGRect rect1 = CGRectMake(m_annot_rect.left * scale,
                              m_annot_rect.top * scale,
    ...
    RDVCanvas *canvas = [[RDVCanvas alloc] init :context :m_scale_pix];
    if (![RDPDFEditNode caret_is_same:pos0 :pos1])
    {
        RDVPage *vpage = [m_layout vGetPage:m_annot_pos.pageno];
        pos1 = [RDPDFEditNode caret_regular_end:pos1];
        pos0 = [m_edit_node caret_regular_start:pos0];
        PDF_RECT rect_word = [m_edit_node getCharRect :pos0];
        pos0 = [m_edit_node getCharNext :pos0];
        long long posn = [RDPDFEditNode caret_regular_end:pos0];
        while (posn < pos1)
        {
            PDF_RECT rect_char = [m_edit_node getCharRect :pos0];
            if ([RDPDFEditNode caret_is_vert:pos0])
            {
                ...
                if (dx * 1.25f > rect_char.right - rect_char.left && dx * 1.25f > rect_word.right - rect_word.left) {
                    ...
                    float dl = [vpage GetVX:rect_word.left] - [m_layout docx];
                    float dr = [vpage GetVX:rect_word.right] - [m_layout docx];
                    float dt = [vpage GetVY:rect_word.bottom] - [m_layout docy];
                    float db = [vpage GetVY:rect_word.top] - [m_layout docy];
                    [canvas FillRect:CGRectMake(dl * scale, dt * scale, (dr - dl) * scale, (db - dt) * scale): GLOBAL.g_sel_color ];
                    rect_word = rect_char;
                }
            } else {
            ...
            pos0 = [m_edit_node getCharNext :pos0];
            posn = [RDPDFEditNode caret_regular_end:pos0];
        }
        float dl = [vpage GetVX:rect_word.left] - [m_layout docx];
        float dr = [vpage GetVX:rect_word.right] - [m_layout docx];
        float dt = [vpage GetVY:rect_word.bottom] - [m_layout docy];
    ...
    {
        if ((m_edit_caret_sta & 2) != 0 && m_edit_caret_rect.right > m_edit_caret_rect.left)
        {
            [canvas FillRect:CGRectMake(m_edit_caret_rect.left * scale, m_edit_caret_rect.top * scale,
                                        (m_edit_caret_rect.right - m_edit_caret_rect.left) * scale,
                                        (m_edit_caret_rect.bottom - m_edit_caret_rect.top) * scale): 0xFF000000];
        }
    }
}
...
-(void)osDrawImages:(CGContextRef)context
{
    if( m_status == sta_image && (m_rects_cnt || m_rects_drawing) )
    {
        PDF_POINT *pt_cur = m_rects;
        PDF_POINT *pt_end = m_rects + (m_rects_cnt<<1);
        if( m_rects_drawing ) pt_end += 2;
        while( pt_cur < pt_end )
        {
            ...
            CGRect rect1 = CGRectMake(rect.left/m_scale_pix,
                                      rect.bottom/m_scale_pix,
                                      (rect.right - rect.left)/m_scale_pix,
                                      (rect.bottom - rect.top)/m_scale_pix);
            CGContextSaveGState(context);
            CGAffineTransform tran = CGAffineTransformMake(1, 0, 0, -1, 0, rect.bottom);
            CGContextConcatCTM(context, tran);
            CGContextDrawImage(context, rect1, m_aimg.CGImage);
...
-(void)pageRotationRefreshContentSize
{
    if(m_save_vmode == 3)
    {
        dispatch_time_t delay = dispatch_time(DISPATCH_TIME_NOW, NSEC_PER_SEC * 0.2);
        dispatch_after(delay, dispatch_get_main_queue(), ^(void){
            [self->m_layout vZoomStart];
            CGPoint point = CGPointMake(self.center.x, self.center.y);
            if ([[[UIDevice currentDevice] systemVersion] floatValue] < 8.0 && [[[UIDevice currentDevice] systemVersion] floatValue] > 6.0 && [[UIDevice currentDevice] userInterfaceIdiom] == UIUserInterfaceIdiomPad)
            {
                CGFloat buffer = point.y;
                point.y = point.x;
                point.x = [self->m_layout vw]/self->m_scale_pix - buffer;
            }
            self->zoomPoint = CGPointMake((point.x - ([self->m_layout vw]/(self->m_scale_pix * 2))), (point.y - ([self->m_layout vh]/(self->m_scale_pix * 2))));
            self->zoomPoint.x = (self->zoomPoint.x < 0) ? (self->zoomPoint.x * -1) : self->zoomPoint.x;
            self->zoomPoint.y = (self->zoomPoint.y < 0) ? (self->zoomPoint.y * -1) : self->zoomPoint.y;
            [self->m_layout vGetPos :(point.x - (self->zoomPoint.x * self.zoomScale)) * self->m_scale_pix :(point.y - (self->zoomPoint.y * self.zoomScale)) * self->m_scale_pix : &self->m_zoom_pos];
            self->m_zoom = self.zoomScale;
            [self->m_layout vZooming:self->m_zoom];
            self.contentSize = CGSizeMake([self->m_layout docw]/self->m_scale_pix, [self->m_layout vGetDoch:self->m_cur_page]/self->m_scale_pix);
            [self->m_layout vSetPos:(point.x - (self->zoomPoint.x * self->m_zoom)) * self->m_scale_pix :(point.y - (self->zoomPoint.y * self->m_zoom)) * self->m_scale_pix :&self->m_zoom_pos];
            self.contentOffset  = CGPointMake([self->m_layout docx]/self->m_scale_pix, [self->m_layout docy]/self->m_scale_pix);
            self->m_status = sta_none;
            [self refresh];
            self.pagingEnabled = GLOBAL.g_paging_enabled && self->m_layout && [self->m_layout vCanPaging];
            [self->m_layout vZoomConfirm];
            CGSize screenSize = [self getScreenFrameForCurrentOrientation];
            CGFloat screenWidth = screenSize.width;
            self->animatePoint = CGPointMake(([self->m_layout vGetPage:self->m_cur_page].GetX - ((screenWidth - [self->m_layout vGetPage:self->m_cur_page].GetWidth) / 2)) / self->m_scale_pix , self.contentOffset.y);
            [self setContentOffset:self->animatePoint animated:NO];
        });
    }
}
...
{
    if([m_edit_node getType] != 1) return;
    if (![RDPDFEditNode caret_is_same:m_edit_pos0 :m_edit_pos1])
    {
    ...
    m_edit_pos0 = [m_edit_node charInsert:m_edit_pos0 :text];
    m_edit_pos1 = m_edit_pos0;
    RDVPage *vpage = [m_layout vGetPage:m_annot_pos.pageno];
    RDPDFPage *page = [vpage GetPage];
    [page setPGEditorNodeModified:YES];
    [m_layout vRenderAsync:m_annot_pos.pageno];
    [self onEditCaret];
    [self ProRedrawOS];
}
...
- (void)deleteBackward
{
    ...
    if (![RDPDFEditNode caret_is_first :m_edit_pos0]) {
        long long pos;
        if ([RDPDFEditNode caret_is_end:m_edit_pos0])
        {
            pos = m_edit_pos0;
            pos &= (~1l);
        } else
...
- (void)OnDoubleTapCheckEnd:(id)sendor
{
    ...
    if( m_annot_pos.pageno >= 0 )
    {
        ...
        if(m_edit_mode)
        {
            m_edit_node = [page getPGEditorNode:m_annot_pos.pdfx: m_annot_pos.pdfy];
        }
        else
        {
        ...
        if(m_edit_node)
        {
            m_annot_rect = [m_edit_node getRect];
            float tmp = m_annot_rect.top;
            m_annot_rect.left = [vpage GetVX:m_annot_rect.left] - [m_layout docx];
            m_annot_rect.top = [vpage GetVY:m_annot_rect.bottom] - [m_layout docy];
            m_annot_rect.right = [vpage GetVX:m_annot_rect.right] - [m_layout docx];
            m_annot_rect.bottom = [vpage GetVY:tmp] - [m_layout docy];
            m_annot_rect0 = m_annot_rect;
            m_status = sta_edit;
            bool on_edge = m_tx < m_annot_rect.left + 8 || m_tx > m_annot_rect.right - 8 ||
                m_ty < m_annot_rect.top + 8 || m_ty > m_annot_rect.bottom - 8;
            if ([m_edit_node getType] == 1 && !on_edge)//text block to display caret
            {
                m_edit_pos1 = [m_edit_node getCharPos:m_annot_pos.pdfx :m_annot_pos.pdfy];
                [self onEditCaret];
                [self caret_show:YES];
                if (m_canvas && m_edit_pos1 != -1)
                    [self becomeFirstResponder];
            } else {
                if (m_canvas) [self resignFirstResponder];
            ...
            [self ProRedrawOS];
        } else if (!m_edit_node && [self isFirstResponder]) {
            [self resignFirstResponder];
        } else if( m_annot )
        {
            m_annot_idx = [m_annot getIndex];
            if(m_del) [m_del OnAnnotTapped:m_annot atPage:m_cur_page atPoint:CGPointMake(x, y)];
            int atype = m_annot.type;
            if ((![self canSaveDocument] || (GLOBAL.g_annot_readonly && [m_annot isReadonly])) && !([self isReadOnlyAnnotEnabled:m_annot]))
            {
                if (m_del) [m_del OnSingleTapped:x:y];
                return;
            }
            ...
            m_status = sta_annot;
            m_aop_attached = false;
            if (atype == 5 || atype == 6 || atype == 16)//it can resize
                m_aop_type = 1;
            else if (atype == 4)
                m_aop_type = 2;
            ...
            m_aop_rect = m_annot_rect;
            [self ProRedrawOS];
            /*
            //following code used for testing.
            if([m_annot fieldType] == 4 && [m_annot getSignStatus] == 0)
            {
                RDPDFDocForm *form = [m_doc newForm];
                RDPDFPageContent *content = [[RDPDFPageContent alloc] init];
                [form setContent:0 :0 :100 :100 :content];
                NSString *fontsFolder = [[NSBundle mainBundle] pathForResource:@"PDFRes" ofType:nil];

                int iret = [m_annot signField:form :[fontsFolder stringByAppendingString:@"/test.pfx"] :@"111111" :@"radaee" :@"" :@"" :@""];
                if(iret == 0)
                {
                    [self setModified:YES force:NO];
...
-(void)OnATouchBegNormal
{
}

-(void)OnATouchBegResize
{
    float mx = (m_annot_rect.left + m_annot_rect.right) * 0.5f;
    float my = (m_annot_rect.top + m_annot_rect.bottom) * 0.5f;
    if ([PDFLayoutView vOnBlk:m_tx :m_ty :m_annot_rect.left :m_annot_rect.top :m_scale_pix])
        m_aop_node = 1;
    else if ([PDFLayoutView vOnBlk:m_tx :m_ty :mx :m_annot_rect.top :m_scale_pix])
        m_aop_node = 2;
    else if ([PDFLayoutView vOnBlk:m_tx :m_ty :m_annot_rect.right :m_annot_rect.top :m_scale_pix])
        m_aop_node = 3;
    else if ([PDFLayoutView vOnBlk:m_tx :m_ty :m_annot_rect.right :my :m_scale_pix])
        m_aop_node = 4;
    else if ([PDFLayoutView vOnBlk:m_tx :m_ty :m_annot_rect.right :m_annot_rect.bottom :m_scale_pix])
        m_aop_node = 5;
    else if ([PDFLayoutView vOnBlk:m_tx :m_ty :mx :m_annot_rect.bottom :m_scale_pix])
        m_aop_node = 6;
    else if ([PDFLayoutView vOnBlk:m_tx :m_ty :m_annot_rect.left :m_annot_rect.bottom :m_scale_pix])
        m_aop_node = 7;
    else if ([PDFLayoutView vOnBlk:m_tx :m_ty :m_annot_rect.left :my :m_scale_pix])
        m_aop_node = 8;
    else if ([PDFLayoutView vInRect:m_tx :m_ty :m_annot_rect])
        m_aop_node = 9;
    else
        m_aop_node = -1;
}

-(void)OnAnnotLoadLine
{
    if (m_aop_attached) return;
    RDVPage *vpage = [m_layout vGetPage:m_annot_pos.pageno];
    m_aop_pt0[0] = [m_annot getLinePoint:0];
    m_aop_pt0[1] = [m_annot getLinePoint:1];
    m_aop_pt0[0].x = [vpage x] - self.contentOffset.x * m_scale_pix + [vpage ToDIBX:m_aop_pt0[0].x];
    m_aop_pt0[0].y = [vpage y] - self.contentOffset.y * m_scale_pix + [vpage ToDIBY:m_aop_pt0[0].y];
    m_aop_pt0[1].x = [vpage x] - self.contentOffset.x * m_scale_pix + [vpage ToDIBX:m_aop_pt0[1].x];
    m_aop_pt0[1].y = [vpage y] - self.contentOffset.y * m_scale_pix + [vpage ToDIBY:m_aop_pt0[1].y];
    m_aop_pts[0] = m_aop_pt0[0];
    m_aop_pts[1] = m_aop_pt0[1];
    m_aop_attached = true;
}
-(void)OnATouchBegLine
{
    [self OnAnnotLoadLine];
    if ([PDFLayoutView vOnBlk:m_tx :m_ty :m_aop_pts[0].x :m_aop_pts[0].y :m_scale_pix])
        m_aop_node = 1;
    else if ([PDFLayoutView vOnBlk:m_tx :m_ty :m_aop_pts[1].x :m_aop_pts[1].y :m_scale_pix])
        m_aop_node = 2;
    else if ([PDFLayoutView vInRect:m_tx :m_ty :m_annot_rect])
        m_aop_node = 3;
    else
...
-(void)OnAnnotLoadPath
{
    if (m_aop_attached) return;
    RDVPage *vpage = [m_layout vGetPage:m_annot_pos.pageno];
    m_annot_type = m_annot.type;
    RDPDFPath *path = nil;
    if (m_annot_type == 7)
    {
        path = [m_annot getPolygonPath];
        m_aop_ppts_cnt = [path nodesCount] - 1;
    }
    else if(m_annot_type == 8)
    {
        path = [m_annot getPolylinePath];
        m_aop_ppts_cnt = [path nodesCount];
    }
    m_aop_ppt0 = (PDF_POINT *)malloc(sizeof(PDF_POINT) * m_aop_ppts_cnt * 2);
    m_aop_ppts = m_aop_ppt0 + m_aop_ppts_cnt;
    PDF_POINT *pcur = m_aop_ppt0;
    for (int cur = 0; cur < m_aop_ppts_cnt; cur++)
    {
        [path node:cur :pcur];
        pcur->x = [vpage x] - self.contentOffset.x * m_scale_pix + [vpage ToDIBX:pcur->x];
        pcur->y = [vpage y] - self.contentOffset.y * m_scale_pix + [vpage ToDIBY:pcur->y];
        pcur++;
    }
    memcpy(m_aop_ppts, m_aop_ppt0, sizeof(PDF_POINT) * m_aop_ppts_cnt);
    m_aop_attached = true;
}

-(void)OnATouchBegPoly
{
    [self OnAnnotLoadPath];
    
    PDF_POINT *pend = m_aop_ppts + m_aop_ppts_cnt;
    m_aop_node = -1;
    for (PDF_POINT *pcur = m_aop_ppts; pcur < pend; pcur++)
    {
        if ([PDFLayoutView vOnBlk:m_tx :m_ty :pcur->x :pcur->y :m_scale_pix])
            m_aop_node = (int)(pcur - m_aop_ppts);
    }
    if (m_aop_node < 0)
    {
        free(m_aop_ppt0);
        m_aop_ppt0 = NULL;
        m_aop_ppts = NULL;
        if ([PDFLayoutView vInRect:m_tx :m_ty :m_annot_rect])
            m_aop_node = 0;
        else
            m_aop_node = -1;
    }
...
-(bool)OnAnnotTouchBegin:(CGPoint)point
{
    ...
    if (![m_annot canMoveAnnot] || (GLOBAL.g_annot_lock && [m_annot isLocked]) || (GLOBAL.g_annot_readonly && [m_annot isReadonly])) return false;
    m_px = point.x * m_scale_pix;
    m_py = point.y * m_scale_pix;
    m_tx = m_px;
    m_ty = m_py;
    if (m_aop_type == 1)
        [self OnATouchBegResize];
    else if (m_aop_type == 2)
        [self OnATouchBegLine];
    else if (m_aop_type == 3)
        [self OnATouchBegPoly];
    else
        [self OnATouchBegNormal];
    return true;
}

-(void)OnATouchMoveNormal
{
    float dx = m_tx - m_px;
    float dy = m_ty - m_py;
    m_aop_rect.left = m_annot_rect.left + dx;
    m_aop_rect.top = m_annot_rect.top + dy;
    m_aop_rect.right = m_annot_rect.right + dx;
    m_aop_rect.bottom = m_annot_rect.bottom + dy;
}

-(void)OnATouchMoveResize
{
    if (m_aop_node < 0) return;
    float dx = m_tx - m_px;
    float dy = m_ty - m_py;
    switch(m_aop_node)
    {
        ...
        case 5:
            m_aop_rect.right = m_annot_rect.right + dx;
            m_aop_rect.bottom += m_annot_rect.right + dy;
            break;
        case 6:
            m_aop_rect.bottom += m_annot_rect.right + dy;
            break;
...
-(void)OnATouchMoveLine
{
    ...
    {
        case 1:
            m_aop_pts[0].x = m_aop_pt0[0].x + dx;
            m_aop_pts[0].y = m_aop_pt0[0].y + dy;
            if (m_aop_pts[0].x > m_aop_pts[1].x)
            {
                m_aop_rect.left = m_aop_pts[1].x;
                m_aop_rect.right = m_aop_pts[0].x;
            }
            else
            {
                m_aop_rect.left = m_aop_pts[0].x;
                m_aop_rect.right = m_aop_pts[1].x;
            }
            if (m_aop_pts[0].y > m_aop_pts[1].y)
            {
                m_aop_rect.top = m_aop_pts[1].y;
                m_aop_rect.bottom = m_aop_pts[0].y;
            }
            else
            {
                m_aop_rect.top = m_aop_pts[0].y;
                m_aop_rect.bottom = m_aop_pts[1].y;
            }
            break;
        case 2:
            m_aop_pts[1].x = m_aop_pt0[1].x + dx;
            m_aop_pts[1].y = m_aop_pt0[1].y + dy;
            if (m_aop_pts[0].x > m_aop_pts[1].x)
            {
                m_aop_rect.left = m_aop_pts[1].x;
                m_aop_rect.right = m_aop_pts[0].x;
...
-(void)OnATouchMovePoly
{
    ...
    if (m_aop_ppt0)
    {
        PDF_POINT *pdst = m_aop_ppts + m_aop_node;
        PDF_POINT *psrc = m_aop_ppt0 + m_aop_node;
        pdst->x = psrc->x + dx;
        pdst->y = psrc->y + dy;
        //update bounding
        m_aop_rect.left = pdst->x;
        m_aop_rect.top = pdst->y;
        m_aop_rect.right = m_aop_rect.left;
        m_aop_rect.bottom = m_aop_rect.top;
        PDF_POINT *pend = m_aop_ppts + m_aop_ppts_cnt;
        for (PDF_POINT *pcur = m_aop_ppts; pcur < pend; pcur++)
        {
            if (m_aop_rect.left > pcur->x) m_aop_rect.left = pcur->x;
            else if (m_aop_rect.right < pcur->x) m_aop_rect.right = pcur->x;
            if (m_aop_rect.top > pcur->y) m_aop_rect.top = pcur->y;
            else if (m_aop_rect.bottom < pcur->y) m_aop_rect.bottom = pcur->y;
        }
    }
    else
    {
...
-(bool)OnAnnotTouchMove:(CGPoint)point
{
    ...
    m_ty = point.y * m_scale_pix;
    if (m_aop_type == 1)
        [self OnATouchMoveResize];
    else if (m_aop_type == 2)
        [self OnATouchMoveLine];
    else if (m_aop_type == 3)
        [self OnATouchMovePoly];
    else
        [self OnATouchMoveNormal];
    [self ProRedrawOS];
    return true;
}

-(void)OnATouchEndNormal
{
    ...
    if( pos.pageno == m_annot_pos.pageno )
    {
        ...
        [actionManger push:[[ASMove alloc] initWithPage:pos.pageno initRect:rect destPage:pos.pageno destRect:m_annot_rect index:m_annot_idx ref:[m_annot getRef]]];
    ...
    else if (m_tx != m_px && m_ty != m_py)
    {
        ...
        if( dpage )
        {
            RDPDFMatrix *mat = [vdest CreateInvertMatrix:self.contentOffset.x * m_scale_pix :self.contentOffset.y * m_scale_pix];
            [mat transformRect :&m_annot_rect];
...
-(void)OnATouchEndResize
{
    if (m_aop_node < 0) return;
    if (m_aop_node != 9)
    {
        m_annot_rect = m_aop_rect;
        RDVPage *vpage = [m_layout vGetPage:m_annot_pos.pageno];
        RDVPos pos;
        [m_layout vGetPos :m_tx :m_ty :&pos];
        RDPDFMatrix *mat = [vpage CreateInvertMatrix:self.contentOffset.x * m_scale_pix :self.contentOffset.y * m_scale_pix];
...
-(void)OnATouchEndLine
{
    ...
    if (m_aop_node != 3)
    {
        ...
        [m_layout vGetPos :m_tx :m_ty :&pos];
        RDPDFMatrix *mat = [vpage CreateInvertMatrix:self.contentOffset.x * m_scale_pix :self.contentOffset.y * m_scale_pix];
        [mat transformPoint:m_aop_pts];
        [mat transformPoint:m_aop_pts + 1];
        [m_annot setLinePoint:m_aop_pts[0].x :m_aop_pts[0].y :m_aop_pts[1].x :m_aop_pts[1].y];
        [self setModifyDateForAnnot:m_annot];
        [self ProUpdatePage :m_annot_pos.pageno];
    }
    else
...
-(void)OnATouchEndPoly
{
    ...
    if (m_aop_ppt0)
    {
        ...
        RDPDFPath *path = [[RDPDFPath alloc] init];
        PDF_POINT *pcur = m_aop_ppts;
        [mat transformPoint:pcur];
        [path moveTo:pcur->x :pcur->y];
        PDF_POINT *pend = m_aop_ppts + m_aop_ppts_cnt;
        for (pcur++; pcur < pend; pcur++)
        {
            [mat transformPoint:pcur];
            [path lineTo:pcur->x :pcur->y];
        }
        if (m_annot_type == 7)
        {
            [path closePath];
...
{
    if (m_status != sta_annot) return false;
    if (![m_annot canMoveAnnot] || (GLOBAL.g_annot_lock && [m_annot isLocked]) || (GLOBAL.g_annot_readonly && [m_annot isReadonly]))
    {
        [self vAnnotEnd];
        return false;
    }
    ...
    m_ty = point.y * m_scale_pix;
    if (m_aop_type == 1)
        [self OnATouchEndResize];
    else if (m_aop_type == 2)
        [self OnATouchEndLine];
    else if (m_aop_type == 3)
        [self OnATouchEndPoly];
    else
        [self OnATouchEndNormal];
    [self vAnnotEnd];
    [self autoSave];
    return true;
}
...
-(bool)OnNoteTouchEnd:(CGPoint)point
{
    ...
    if( vpage )
    {
        ...
        if( page )
        {
            ...
            [self setModifyDateForAnnot:[page annotAtIndex:(page.annotCount - 1)]];
...
-(bool)OnEditTouchBegin
{
    ...
    if (m_edit_node)
    {
        ...
        if ([m_edit_node getType] == 1 && !on_edge)//text block to display caret
        {
            m_edit_pos0 = [m_edit_node getCharPos :m_annot_pos.pdfx :m_annot_pos.pdfy];
            m_edit_pos1 = m_edit_pos0;
            PDF_RECT crect = [m_edit_node getCharRect :m_edit_pos0];
            if ([RDPDFEditNode caret_is_vert :m_edit_pos0])//vertical writing mode.
            {
                crect.left = [vpage GetVX:crect.left] - [m_layout docx];
                crect.right = [vpage GetVX:crect.right] - [m_layout docx];
                tmp = (crect.right - crect.left) * 0.1f;
                if ([RDPDFEditNode caret_is_end :m_edit_pos0])
                {
                    crect.top = [vpage GetVY:crect.bottom] - [m_layout docy] - tmp * 0.5f;
            ...
            {
                tmp = crect.top;
                crect.top = [vpage GetVY:crect.bottom] - [m_layout docy];
                crect.bottom = [vpage GetVY:tmp] - [m_layout docy];
                tmp = (crect.bottom - crect.top) * 0.1f;
                if ([RDPDFEditNode caret_is_end :m_edit_pos0]) {
                    crect.left = [vpage GetVX:crect.right] - [m_layout docx] - tmp * 0.5f;
                    crect.right = crect.left + tmp;
                } else {
                    crect.left = [vpage GetVX:crect.left] - [m_layout docx] - tmp * 0.5f;
                    crect.right = crect.left + tmp;
                }
            }
...
-(void)onEditCaret
{
    PDF_RECT crect = [m_edit_node getCharRect:m_edit_pos1];
    float tmp;
    RDVPage *vpage = [m_layout vGetPage:m_annot_pos.pageno];
    if ([RDPDFEditNode caret_is_vert:m_edit_pos1])//vertical writing mode.
    {
        crect.left = [vpage GetVX:crect.left] - [m_layout docx];
        crect.right = [vpage GetVX:crect.right] - [m_layout docx];
        tmp = (crect.right - crect.left) * 0.1f;
        if ([RDPDFEditNode caret_is_end:m_edit_pos1])
        {
            crect.top = [vpage GetVY:crect.bottom] - [m_layout docy] - tmp * 0.5f;
            crect.bottom = crect.top + tmp;
        } else {
            crect.top = [vpage GetVY:crect.top] - [m_layout docy] - tmp * 0.5f;
            crect.bottom = crect.top + tmp;
        }
    } else//horizon writing mode
    {
...
-(bool)OnEditTouchMove:(CGPoint)point
{
    if (!m_edit_mode || !m_edit_node) return false;
    int nx = point.x * m_scale_pix;
    int ny = point.y * m_scale_pix;
    RDVPos pos;
    ...
    if (pos.pageno == m_annot_pos.pageno) {
        ...
        {
            m_annot_rect.left = m_annot_rect0.left + nx - m_tx;
            m_annot_rect.top = m_annot_rect0.top + ny - m_ty;
            m_annot_rect.right = m_annot_rect0.right + nx - m_tx;
            m_annot_rect.bottom = m_annot_rect0.bottom + ny - m_ty;
        } else {
            m_edit_pos1 = [m_edit_node getCharPos :pos.pdfx :pos.pdfy];
            [self onEditCaret];
        }
    }
    return true;
...
-(void)OnNoneTouchBegin:(CGPoint)point :(NSTimeInterval)timeStamp
{
    ...
    m_px = m_tx;
    m_py = m_ty;
    [self OnEditTouchBegin];
    if (m_edit_mode) [self ProRedrawOS];
}

-(void)OnNoneTouchMove:(CGPoint)point :(NSTimeInterval)timeStamp
{
    if([self OnEditTouchMove:point])
    {
        m_px = point.x * m_scale_pix;
        m_py = point.y * m_scale_pix;
        [self ProRedrawOS];
...
-(void)OnNoneTouchEnd:(CGPoint)point :(NSTimeInterval)timeStamp
{
    ...
    if( timeStamp - m_tstamp_tap < 0.20 )//single tap
    {
        ...
        else if([self OnEditTouchMove:point])
        {
            m_doubleTapCount = 0;
            m_px = point.x * m_scale_pix;
            m_py = point.y * m_scale_pix;
            ...
            m_edit_node = [page getPGEditorNode:m_annot_pos.pdfx :m_annot_pos.pdfy];
            PDF_RECT nrect;
            nrect.left = [vpage ToPDFX: m_annot_rect.left :[m_layout docx]];
            nrect.top = [vpage ToPDFY: m_annot_rect.bottom :[m_layout docy]];
            nrect.right = [vpage ToPDFX: m_annot_rect.right :[m_layout docx]];
            nrect.bottom = [vpage ToPDFY: m_annot_rect.top :[m_layout docy]];
            [m_edit_node setRect :nrect];
            [page setPGEditorNodeModified:YES];
...
- (void)vAddImageWithImage:(UIImage *)image withRect:(PDF_RECT)rect
{
    ...
    RDPDFDocImage *dimg = [m_doc newImage:ref :YES :YES];
    
    // Add the image
    [page addAnnotBitmap:dimg :&rect];
    [self setModifyDateForAnnot:[page annotAtIndex:(page.annotCount - 1)]];
...
- (BOOL)PDFSignField:(RDVPage *)vp :(RDPDFAnnot *)annot
{
    ...
    CGImageRef ref = [image CGImage];
    RDPDFDocImage *docImage = [m_doc newImage:ref :YES :YES];
...
{
    if( m_status != sta_sel || !m_sel ) return false;
    bool ret = [m_sel Remove];
    if(ret)
    {
        [self setModified:true force:NO];
        [self autoSave];
        [self ProUpdatePage :m_sel_pos.pageno];
        [self setNeedsDisplay];
    }
...
-(void)vAnnotRemove
{
    ...
    if (GLOBAL.g_annot_lock && [m_annot isLocked]) return;
...
-(void)removeAnnot :(RDPDFAnnot *)annot
{
    [annot removeFromPage];
    [self setNeedsDisplay];
    [m_canvas setNeedsDisplay];
        ...
        while( pt_cur < pt_end )
        {
            ...
            if( pos.pageno >= 0 )
            {
                ...
                RDPDFMatrix *mat = [vpage CreateInvertMatrix:self.contentOffset.x * m_scale_pix
                                                          :self.contentOffset.y * m_scale_pix];
                [mat transformRect:&rect];
                if (!m_dimg) m_dimg = [m_doc newImage2:m_aimg.CGImage :0 :YES];
                [page addAnnotBitmap:m_dimg :&rect];
...
-(void)caret_show:(bool)show
{
    if ((m_edit_caret_sta & 1) != 0 && !show) {
        m_edit_caret_sta &= (~3);
        [m_edit_timer invalidate];
        m_edit_timer = nil;
        m_edit_caret_rect.left = 0;
        m_edit_caret_rect.top = 0;
        m_edit_caret_rect.right = 0;
    ...
    if ((m_edit_caret_sta & 1) == 0 && show) {
        m_edit_caret_sta |= 3;
        m_edit_timer = [NSTimer scheduledTimerWithTimeInterval:0.5 repeats:YES block:^(NSTimer * _Nonnull timer) {
            [self performSelectorOnMainThread:@selector(updateCaret) withObject:self waitUntilDone:NO];
        }];
    }
}
...
-(void)vEditStart
{
    ...
    m_edit_mode = true;
    [self caret_show:true];
}
-(void)vEditCancel
{
    if (!m_edit_mode) return;
    //close IME
    //if (m_eMenu) m_eMenu.hide();
    if (m_canvas) [self resignFirstResponder];
    int pcur;
    ...
    for (pcur = 0; pcur < pcnt; pcur++) {
        ...
        page = nil;
        if (ret)//mean this has modified.
        {
            [m_layout vRenderAsync:pcur];
            if(m_del) [m_del OnPageUpdated:pcur];
        }
    }
    [self caret_show:false];
    m_edit_pos0 = -1;
    m_edit_pos1 = -1;
    m_edit_node = nil;
    m_edit_mode = false;
