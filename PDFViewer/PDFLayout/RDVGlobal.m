...
+ (void)Init
{
    int active_type = Global_active(g_serial);
    ...
    [[NSUserDefaults standardUserDefaults] setBool:active_type > 0 forKey:@"actIsActive"];
    ...
    int fcnt = Global_getFaceCount();
    for(int fcur = 0; fcur < fcnt; fcur++)
    {
        NSLog(@"font - %s", Global_getFaceName(fcur));
    }
    ...
    [RDPDFEditNode setDefFont:@"Arial"];
    [RDPDFEditNode setDefCJKFont:@"AR PL SungtiL GB"];//for CJK
...
- (void)setup {
    ...
    _g_readonly = false;
    _g_fake_sign = false;
    _g_disable_text_copy = false;
    _g_zoomed_stop_on_boundaries = false;
