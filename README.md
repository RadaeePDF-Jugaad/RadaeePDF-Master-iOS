...
4. **Apple Silicon Simulators** (no configuration needed)
   - The SDK ships as `RDDLib.xcframework`, which includes a native `arm64` slice for the iOS Simulator, so it builds and runs directly on Apple Silicon (M1/M2/M3) Macs.
...
1. Download the RadaeePDF SDK from [https://www.radaeepdf.com/](https://www.radaeepdf.com/)
2. Drag the `PDFLib` folder into your Xcode project
3. Add `RDDLib.xcframework` to your target's **Build Phases** → **Link Binary With Libraries**
